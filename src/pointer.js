...
export class Pointer {
  constructor(scene, renderer, camera, opts) {
    this.listeners = {};
    this.opts = opts || {};
    this.opts.enableLaser =
      opts.enableLaser !== undefined ? opts.enableLaser : true;
    this.opts.laserLength =
      opts.laserLength !== undefined ? opts.laserLength : 3;
    this.opts.enableMoveEvents =
      opts.enableMoveEvents !== undefined ? opts.enableMoveEvents : true;
    this.scene = scene;
    ...
    this.controller1.addEventListener(
      'selectstart',
      this.controllerSelectStart.bind(this)
    );
    this.controller1.addEventListener(
      'selectend',
      this.controllerSelectEnd.bind(this)
    );
    ...
    if (this.opts.enableLaser) {
      ...
      geometry.addAttribute(
        ...
        new THREE.Float32BufferAttribute(
          [0, 0, 0, 0, 0, -this.opts.laserLength],
          3
        )
      );
      geometry.addAttribute(
        'color',
        new THREE.Float32BufferAttribute([1.0, 0.5, 0.5, 0, 0, 0], 3)
      );
  ...
  touchEnd(e) {
    ...
    const intersects = this.raycaster
      ...
      .filter((it) => this.intersectionFilter(it.object));
    intersects.forEach((it) => {
      this.fire(it.object, POINTER_RELEASE, {
  ...
  _processClick() {
    ...
    if (intersects.length > 0) {
      const it = intersects[0];
      this.fire(it.object, POINTER_CLICK, {
        type: POINTER_CLICK,
        point: it.point,
        intersection: it
      });
  ...
  mouseDown(e) {
    ...
    const intersects = this.raycaster
      ...
      .filter((it) => this.intersectionFilter(it.object));
    intersects.forEach((it) => {
      this.fire(it.object, POINTER_PRESS, {
        type: POINTER_PRESS,
        point: it.point,
        intersection: it
      });
  ...
  setMouseSimulatesController(val) {
    ...
    if (this.opts.mouseSimulatesController) {
      ...
      this.controller1.quaternion.setFromUnitVectors(
